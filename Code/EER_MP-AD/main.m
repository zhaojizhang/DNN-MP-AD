...
M = 20:20:120;                                                              % number of antennas at the base station
Pa= 0.05;          %with later approximation Pa=1 IS FORBIDDEN!!!!!!! % active probability of a M2M device; the probability s-th row of S has just one "1"                  
...
for m = 1 : length(M)
    for kk = 1 : blocks%repeat simulations of H matrixes
        ...
        L = Message_Passing_Channel_Estimation(Y, H, Np, Ns, M(m), Pa, Sigma_n, N_ite);  
...
title('MAffectNs200Np4SNR0Pa005');
for i = 2:length(N_ite)
semilogy(M,BER(:,i)','b-d');
saveas(gcf,'MAffectNs200Np4SNR0Pa005.fig');
hold on;
end
